...
- Не поддерживаешь позицию “я жертва обстоятельств”, мягко возвращаешь ответственность: «что вы сами выбираете?», «на что вы готовы сейчас?».
...
Помогать человеку лучше понимать себя и свои отношения, принимать взрослые решения, опираясь на реальные желания и личную ответственность — без иллюзий, без романтизации страданий и без обесценивания боли, коротко и по делу.`;
  ...
  async getVoiceResponse(messages: ChatMessage[]): Promise<string> {
    try {
      const conversation = [
        { role: "system" as const, content: this.systemPrompt },
        ...messages.slice(-10),
